...
## 📝 Your Notes
```javascript
// Why it is important to have a cleanup function for a fetch in an useEffect

    // If the component mounts/unmounts 100 times, we'll have 100 concurrent fetch requests running! system overload
...
// async/await in an useEffect
    // you cannot return  an async function or a promise from an useEffect, but a cleanup function
    // The reason this doesn't work is because when you make a function async, it
        ...
        or explicitly returning a function).
    // react does not know how to handle a promise as a return value of an useEffect
    // by returning a promise instead of a clean up function, the effect cannot be cleaned up anymore
    // Design Pattern for the async function -> React.useEffect(() => {
 ...
 //Error Boundary
 // it can be used if your application throws an error during rendering, lyfecycle methods or 
 // constructors, not through asynchronous calls like promises. 
 // Promises and their rejections are not considered errors in JS.       
